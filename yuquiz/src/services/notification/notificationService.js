...
// 알림 리스트 조회
const getNotifications = async (page = 0, sort = "DATE_DESC", view = "UNCHECKED") => {
  try {
    ...
    if (error.response) {
      if (error.response.status === HttpStatusCode.Forbidden) {
        ...
        throw new Error("알림 목록을 불러오는 중 문제가 발생했습니다. 다시 시도해주세요.");
      }
    } else {
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === HttpStatusCode.BadRequest) {
        ...
        throw new Error("알림 읽음 처리 중 문제가 발생했습니다. 다시 시도해주세요.");
      }
    } else {
      throw new Error("서버와 연결할 수 없습니다.");
    }
